...
class ClientBetweenRegions(StatesGroup):
    region_level1 = State()
    region_level2 = State()
    region_level3 = State()
    region_level4 = State()
    ...
    region_level8 = State()
    region_level9 = State()
    region_level10 = State()
    region_level11 = State()
    region_level12 = State()
    ...
    async def menu_to_region(self, call: types.CallbackQuery, state: FSMContext):
        ...
        async with state.proxy() as data:
            dta = call.data.split('_')
            Text_lang = Txt.language[data.get('lang')]
            question = Text_lang.questions.passenger.to_region
            if dta[0] == "town":
    ...
    async def _select_towns(self):
        towns = await pg.select_between_regions(from_region=self.__data.get("from_region"),
    ...
    def register_handlers_client_between_regions(self, dp: Dispatcher):
        dp.register_callback_query_handler(self.menu_from_region, text='out',                                           state=Client.client_level1)
        ...
        dp.register_callback_query_handler(self.menu_to_region, lambda x: x.data.startswith("town"),                    state=self.region_level2)
        dp.register_callback_query_handler(self.menu_to_region, text='back',                                            state=self.region_level4)

        dp.register_callback_query_handler(self.menu_date, lambda x: x.data.startswith("region"),                       state=self.region_level3)
        dp.register_callback_query_handler(self.menu_date, text='back',                                                 state=self.region_level5)

        dp.register_callback_query_handler(self.menu_time, lambda x: x.data.startswith("date"),                         state=self.region_level4)
        dp.register_callback_query_handler(self.menu_time, text='back',                                                 state=self.region_level6)

        dp.register_callback_query_handler(self.menu_time_change, lambda x: x.data and x.data.startswith("time"),       state=self.region_level5)

        dp.register_callback_query_handler(self.menu_number, text='continue',                                           state=self.region_level5)
        dp.register_callback_query_handler(self.menu_number, text='back',                                               state=self.region_level7)

        dp.register_callback_query_handler(self.menu_to_town, lambda x: x.data.startswith("num"),                       state=self.region_level6)
        dp.register_callback_query_handler(self.menu_to_town, text='back',                                              state=self.region_level8)

        dp.register_callback_query_handler(self.menu_zero_town, text='zeroTown',                                        state=self.region_level8)

        dp.register_callback_query_handler(self.menu_model, lambda x: x.data.startswith("town"),                        state=self.region_level7)
        dp.register_callback_query_handler(self.menu_model, text='back',                                                state=self.region_level9)

        dp.register_callback_query_handler(self.menu_distinct_car, lambda x: x.data and x.data.startswith("car"),       state=self.region_level8)
        dp.register_callback_query_handler(self.menu_distinct_car, text='back',                                         state=self.region_level10)
        #
        dp.register_callback_query_handler(self.menu_order, lambda x: x.data and x.data.startswith("order"),            state=self.region_level9)
        dp.register_callback_query_handler(self.menu_order, text='back',                                                state=self.region_level11)

        dp.register_callback_query_handler(self.menu_booking, lambda x: x.data and x.data.startswith("book"),           state=self.region_level10)
        #

        dp.register_message_handler(self.menu_phone_text, content_types=["text"],                                       state=self.region_level11)
        dp.register_message_handler(self.menu_phone_contact, content_types=["contact"],                                 state=self.region_level11)
